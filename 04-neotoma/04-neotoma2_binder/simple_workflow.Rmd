...
```{r simpleTaxonChange, eval=FALSE, eval=FALSE}
allSamp <- allSamp %>% 
...
```{r PlotTaxonCounts, echo=FALSE, eval=FALSE, fig.cap="**Figure**. A plot of the number of sites a taxon appears in, against the number of samples a taxon appears in.", message=FALSE}
taxaplots <- taxa(mich_dl)
...
```{r translationDisplay, message=FALSE, echo = FALSE, eval=FALSE}
translation <- readr::read_csv("data/taxontable.csv")
...
```{r harmonizationTableOut, message = FALSE, echo=FALSE, eval=FALSE}
...
```{r summarizeByTime, message = FALSE, eval=FALSE}
plottingTaxa <- taxa(plottingSite) %>%
  filter(ecologicalgroup %in% c("TRSH")) %>%
  filter(elementtype == "pollen") %>%
  arrange(desc(sites)) %>% 
...
```{r makeSamplesSpatial, eval=FALSE}
modern <- samples(mich_dl) %>% 
  filter(age < 50) %>% 
