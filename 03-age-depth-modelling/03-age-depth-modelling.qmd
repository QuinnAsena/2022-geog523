...
theme:
  ...
  dark: darkly
---
### Authors: Quinn Asena & Jack Williams
...
4. Calculate the difference between the original ^14^C age and the median calibrated age estimate.
=======
...
6. For a radiocarbon age estimate of 10000 years BP with a standard deviation of 100 years, calibrate it to three different calibration curves.
...
- Look at the Devils Lake pollen diagram in Neotoma Explorer and pick two notable vegetation events (e.g. end-Pleistocene *Picea* decline). Then, identify the depths of these events in Neotoma Explorer and use your output from clam to build a data matrix that reports the inferred mean age and 95% CI of these events in separate columns. Your data matrix should have 9 columns: Depth, MeanAge_LinInterp, 95%CI_LinInterp, MeanAge_LinReg, 95%CI_LinReg, MeanAge_Poly, 95%CI_Poly, MeanAge_Spline, 95%CI_Spline.

4. Using clam, construct a smooth spline (type="smooth") with the default smoothing parameter. Experiment with alternative values of the smooth parameter and pick 3-4 that show a range of model behavior.  ake age-depth plots to show the various splines.
=======
:::{.callout-warning}
## Homework
