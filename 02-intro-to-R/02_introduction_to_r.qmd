...
This workbook is intended to provide you with some basics skills, but primarily to direct you to relevant resources. Links will be provided along the way and in @sec-resources are links to resources (some of which this lab is based on) that include everything needed for this module. 
...
Typicall we work with 'objects' in R. An object can be a value, word, data, matrix, or any acceptable class in R. Objects are stored in the 'environment' (by default, the top right panel) in RStudio. To create an object we use the 'assignment'operaror (`<-`). For details [see here](https://datacarpentry.org/R-genomics/01-intro-to-R.html#creating_objects).
...
*Note object names cannot have spaces, use snake_case or CamelCase*
...
There are many 'functions' built into R to make life easier. Functions are easy to recognise because it is a function name followed by parentheses. Everything put inside the parentheses are called 'arguments'. A function takes an input argument (or several) and produces an output. For example:

```{r functions}
# Take the square root of the number 16
sqrt(16) # '16' is the 'argument' provided to the function sqrt
sqrt(radius) # Take the square root of the object defined earlier
round(area) # Round up the result of area 

# Note that we are performing calculations with the object but not overwriting the object.
# The objects radius and area still retain their original values.
# we could overwrite the object like this:
radius <- sqrt(radius) # run the object radius again to see the new value
# or create a new one like this:
area_round <- round(area)
```

[See here](https://datacarpentry.org/R-genomics/01-intro-to-R.html#functions) for more detail.
...
R has a set of functions built in like the examples above but often we need more that come from downloading packages. A package is a collection of functions. Packages are often specialized to complete particular tasks and analyses. A lot of learning R is figureing out the functions you need.
...
One of the most important concepts to familiarise youself with is data types and structures. The objects we worked with `radius` and `area` were both type numeric. Other types include: characters (also called strings),

```{r types}
char <- "this is a string"
int <- 42L # Integers
doub <- 1.61803 # Decimals called type double
logic <- TRUE # Logical TRUE or FALSE values (case sensitie)
```

To find the type of an object use the function `typeof()`. Data structures are organised sets of values. The most fundamental are:
...
Other structures exists such as lists and arrays which we will leave alone for now.
...
factor_vec <- factor(c("small", "medium", "large", "large")) # a special type of vector of categories
...
Some of the fundamental structures are well described [here](https://environmentalcomputing.net/getting-started-with-r/data-types-structure/).

# Homework

If you are unfamiliar with R, go through this workbook again and link through to the resources provided along the way. Each link points to a useful recourse for each topic (assignment, objects, data structures...) that go into more detail.
