...
```{r patch, message=FALSE, echo=FALSE, warning=FALSE, fig.height=10, fig.cap = "Example of visualising data from penguins"}
...
![RStudio interface](./images/rstudio_session_4pane_layout.png)
...
![Data structures](./images/Data_types_structure_image.jpg)
...
To finish up with something fun, let's check-out the penguins data shown at the start. The `palmerpenguins` package has a data frame called `penguins` built-in. To load the data use:
...
```{r ExamplePlot, message=FALSE, warning=FALSE, results=FALSE}
