...
# We will use the rioja package later in the course.
...
Now R knows we want to access functions from the rioja package. More on this later in the course.
...
The `radius` and `area` objects are vector objects of length 1. Other structures exists such as lists and arrays which we will leave alone for now. The function `c()` concatenates (sticks together) multiple values that must be of the same type into a vector.
...
The `data.frame()` function can combine the vectors into a data frame. A dataframe can store data of mixed types. That is, each column can have a different type, you may have a column of strings (e.g., species names), and a column of values belonging to each species (e.g., abundances).
