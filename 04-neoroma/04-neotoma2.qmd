...
The neotoma package provides a series of functions inside of R, each one of which calls one or more APIs. neotoma was primarily written by Simon Goring, with support from NSF-Geoinformatics and the ROpenSci project. Let’s begin by loading the neotoma package into RStudio.
...
`get_sites()` (and most `neotoma2` functions) returns an data object of type `sites`, his also allows us to use some of the other `neotoma2` functions more easily. In RStudio, use the Environment panel in upper right to explore the contents of `samwell_site`.
...
The neotoma data structure is as follows (@fig-neostructure):

- sites: smltiple locations (e.g., several lakes)
...
`get_datasets()` returns a list of datasets containing the metadata for each dataset. It can receive as inputs vectors of site names, vectors of site IDs, or data objects of class site (i.e. output from get_site!). For example:
...
You can use `get_publications()` to get more information. Pass in the publication IDs or a dataset object, e.g.: 
```{r, eval=FALSE}
...
## Downloading data `get_downloads()` and `get_geochron()`

`get_downloads()` returns a list of download objects, one for each retrieved dataset. Note that `get_downloads()` returns the actual data associated with each dataset, rather than a list of the available datasets, as in `get_datasets()` above.

`get_downloads()` will accept an object of `class` dataset (e.g., `samwell_dataset`) or of `class` site (e.g., `samwell_site`). If the latter, it will automatically query for the datasets associated in each site. For example:
...
Collection units contain datasets (@fig-neostructure). We can use the `datasets()` function on a site (or sites) object return details on what records they contain:
...
Now, we can use get downloads to download a single dataset, rather that _all_ the data as we did above. Let’s look a bit more closely at the data frame returned by `get_downloads()`, using the just the vertebrate datasets for Samwell Cave Popcorn Dome (dataset 14262):
